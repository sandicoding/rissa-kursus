...
$bayar = $pdo->query("SELECT * From pemesanan WHERE month(created_at) = '$bln' AND year(created_at) = '$thn' AND tipe='$tipe' ORDER BY idpesan ASC");
$row1 = $bayar->fetch();
$pdf->SetFont('Arial','B',12);
...
$bayar1 = $pdo->query("SELECT * From pemesanan WHERE month(created_at) = '$bln' AND year(created_at) = '$thn'  AND tipe='$tipe' ORDER BY idpesan ASC");
...
$bayar2 = $pdo->query("SELECT SUM(harga) as total From pemesanan WHERE month(created_at) = '$bln' AND year(created_at) = '$thn'  AND tipe='$tipe' ORDER BY idpesan ASC");
$data = $bayar2->fetch();
$pdf->Cell(10,9,'',0,1);
